...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Fetch file report and delete orphaned media assets from Immich."
    )
    parser.add_argument(
        "--api_key", help="Immich API key for authentication", nargs="?", default=None
    )
    parser.add_argument(
        "--api_url",
        help="Full address for Immich, including protocol and port",
        nargs="?",
        default=None,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--stack_method", help="JPGwithRAW, RAWwithJPG", nargs="?", default="JPGwithRAW"
    )
    ...
    for item in get_criteria_config():
        ...
        if "regex" in item.keys():
            ...
            if match:
                ...
                raise Exception(
                    f"Match not found for value: {value}, regex: {regex_key}"
                )
            else:
                return []
...
class Immich:
    ...
    def fetchAssets(self, size: int = 1000) -> list:
        ...
        while payload["page"] is not None:
            ...
            response = session.post(
                f"{self.api_url}/search/metadata", headers=self.headers, json=payload
            )
    ...
    def removeOfflineFiles(self, library_id: str) -> None:
        ...
        response = session.post(
            f"{self.api_url}/libraries/{library_id}/removeOffline", headers=self.headers
        )
    ...
    def modifyAssets(self, payload: dict) -> None:
        ...
        response = session.put(
            f"{self.api_url}/assets", headers=self.headers, json=payload
        )
    ...
    for i, v in enumerate(stacks):
        ...
        if args.skip_previous:
            ...
            if len(children_id) == 0:
                ...
                logger.info(
                    f'   Parent name: {stack[0]["originalFileName"]} ID: {parent_id}'
                )
                for child in stack[1:]:
                    logger.info(
                        f'   Child name:  {child["originalFileName"]} ID: {child["id"]}'
                    )
